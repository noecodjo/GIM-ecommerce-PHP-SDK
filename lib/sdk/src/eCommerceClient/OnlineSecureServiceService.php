...
class OnlineSecureServiceService extends \SoapClient
{
  ...
  $options = array_merge(array ('stream_context' => stream_context_create(array('ssl'=> array('verify_peer'=>false,'verify_peer_name'=>false))),), $options);

  /***********************************************************/
      $options = array_merge(array ('features' => 1,), $options);
      if (!$wsdl) {
      // $wsdl = dirname(__FILE__) . DIRECTORY_SEPARATOR . 'paymentService.wsdl.xml';
        $wsdl = 'https://gim.barakamoney.net/PgWebService/services/OnlineSecureService?wsdl';
        //$wsdl = _ECOMM_FRONT_PROTOCOL_.'://'._ECOMM_FRONT_SERVER_.':'._ECOMM_FRONT_PORT_.'/PgWebService/services/OnlineSecureService?wsdl';
       // $wsdl = _ECOMM_FRONT_PROTOCOL_.'://'._ECOMM_FRONT_SERVER_.'/PgWebService/services/OnlineSecureService?wsdl';

      }

    //  echo "WSDL VALUE = ".$wsdl;
